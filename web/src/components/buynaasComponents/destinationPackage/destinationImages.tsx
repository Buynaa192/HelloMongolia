    ...
    <section className="px-4 md:px-10 py-6 text-white mt-10 ">
      ...
      <div className="grid grid-cols-4 grid-rows-2 gap-2 h-[400px] overflow-hidden rounded-xl">
        <div className="row-span-2 col-span-2 relative">
          <Image src={destination.destinationImages[1]} fill alt={destination.destinationName} className="w-full h-full object-cover rounded-xl" />
        </div>

        {destination.destinationImages.slice(0, 3).map((item, index) => (
          <Image key={index} width={500} height={500} alt={destination.destinationName} src={item} className="w-full h-full object-cover rounded-xl" />
        ))}

        <div className="relative">
          <Image
            fill
            src={destination.destinationImages[3] || destination.destinationImages[2] || destination.destinationImages[0]}
          ...
          <div className="absolute bottom-2 right-2">
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <button className="bg-white/80 backdrop-blur-md text-black px-3 py-1 text-sm rounded-md font-semibold hover:bg-white">Show All Photos</button>
              </DialogTrigger>
