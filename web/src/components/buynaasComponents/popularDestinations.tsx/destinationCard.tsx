    ...
    <Link href={`/explore-destinations/${region}/${item._id}`}>
      <div className="w-full h-100 relative group overflow-hidden flex flex-col  rounded-lg  gap-2">
        ...
        <div className="w-full h-[600px] relative rounded-lg flex items-center justify-center">
          ...
          <span className="absolute text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 text-lg">See more</span>
        </div>

        <p className="relative z-10 text-white text-2xl font-bold text-center ">{item.destinationName}</p>
