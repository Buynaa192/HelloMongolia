      ...
      <div className="justify-between hidden gap-2 lg:flex">
        {headers.map(({ name, url }) => (
          <Link key={name} href={url} className="text-white">
            <Button variant={pathname === url ? "secondary" : "ghost"} className="font-medium cursor-pointer">
              {name}
            </Button>
          </Link>
        ))}
        ...
        <div className="absolute h-10 right-4">
          <Popover>
            <PopoverTrigger className="relative w-10 h-10 overflow-hidden border rounded-full cursor-pointer">
              <Image src={company.AvatarImage} alt="Company Avatar" fill className="block object-cover" />
            </PopoverTrigger>
            <PopoverContent align="end" className="p-2 border-none shadow-2xl w-[240px]">
              <div className="flex flex-col gap-2">
                <Link href={"/company/AllPackages"}>
                  <Button className="justify-start w-full cursor-pointer" size="lg">
                    <LayoutDashboardIcon size={16} />
                    <p>Dashboard</p>
                  </Button>
                </Link>

                <Button onClick={signOut} className="justify-start w-full cursor-pointer">
                  <LogOut size={16} />
                  <p>Log out</p>
                </Button>
              </div>
